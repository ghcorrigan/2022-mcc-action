...
fileCheck = cell2mat(cellfun(@(x) contains(x,[region 'SDFDiff_workspaceA102dirPNCDirVal3kstrict']),fileNames,'uni',0));
if any(fileCheck)
...
parfor neuron_i = 1:size(mcc_map_info,1)
    ...
    [glm_out_mcc{neuron_i,1}, encoding_flag_mcc(neuron_i,:), ...
        inh_pNC{neuron_i,1}] =...
        stopping_diffSDF_fr_alignedStrict(neuron_i, mcc_map_info, behavior(behaviorIdx,:));
end
% 
% parfor neuron_i = 1:size(mcc_map_info,1)
...
parfor neuron_i = 1:size(mcc_map_info,1)
    ...
    trls2nan = true(size(z_sdf,1),1);   trls2nan(goodTrls) = false;
    z_sdf(trls2nan,:) = nan(sum(trls2nan==true),3001);
    ...
    save(['E:\conflict\results\' region 'SDFDiff_workspaceA102dirPNCDirVal3kstrict.mat'])
...
mcc_analysis_table.SSDs = cellfun(@(x) x(2,:),inh_pNC,'uni',0);
