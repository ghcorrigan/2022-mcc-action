...
fileCheck = cell2mat(cellfun(@(x) contains(x,[region 'regression_workspaceA102dir']),fileNames,'uni',0));
if any(fileCheck)
...
parfor neuron_i = 1:size(mcc_map_info,1)
    ...
    [glm_out_mcc{neuron_i,1}, encoding_flag_mcc(neuron_i,:), ...
        encoding_beta_mcc(neuron_i,:),inh_pNC{neuron_i,1},frs{neuron_i,1},sig_bins{neuron_i}] =...
        stopping_glm_func_fr_aligned(neuron_i, mcc_map_info, dataFiles_beh, dirs,behavior(behaviorIdx,:));
end
...
parfor neuron_i = 1:size(mcc_map_info,1)
    ...
    [glm_out_mccB{neuron_i,1}, encoding_flag_mccB(neuron_i,:), encoding_beta_mccB(neuron_i,:)] =...
        stopping_glm_func_fr_aligned_baseline(neuron_i, mcc_map_info, dataFiles_beh, dirs,behavior(behaviorIdx,:));
end
parfor neuron_i = 1:size(mcc_map_info,1)
    neuralFilename = mcc_map_info.session{neuron_i};
     behFilename = data_findBehFile(neuralFilename);
...
parfor neuron_i = 1:size(mcc_map_info,1)
    ...
    save(['E:\conflict\results\' region 'regression_workspaceA102dir.mat'])
