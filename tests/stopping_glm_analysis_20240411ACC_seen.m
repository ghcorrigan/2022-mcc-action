...
fileCheck = cell2mat(cellfun(@(x) contains(x,[region 'seenregression']),fileNames,'uni',0));
if any(fileCheck)
...
parfor neuron_i = 1:size(mcc_map_info,1)
    ...
    [glm_out_mcc{neuron_i,1}, encoding_flag_mcc(neuron_i,:), ...
        encoding_beta_mcc(neuron_i,:),remove(neuron_i)] =...
        stopping_glm_func_fr_seen(neuron_i, mcc_map_info, dataFiles_beh, dirs,...
        behavior(behaviorIdx,:));
...
goodUnits = find(sum(encoding_flag_mcc(:,[1,4]),2)==2);

%% Extract stop-related spike density functions
parfor neuron_i = 1:height(mcc_map_info)
    
    neuralFilename = mcc_map_info.session{neuron_i};
    neuronLabel = mcc_map_info.unit{neuron_i};
    fprintf('Extracting: %s - %s ... [%i of %i]  \n',neuralFilename,...
        neuronLabel,neuron_i,size(mcc_map_info,1))
    ...
    statData = load(['E:\conflict\results\DATA_SPK_STATIONARITY\Ben\stationarityData_'...
        mcc_map_info.probeName{neuron_i}]);
    ndata = load(['E:\conflict\results\probeTrls\neuronData_'...
        mcc_map_info.probeName{neuron_i}]);
    tempInd = find(contains(ndata.neuronData.DSPname,neuronLabel));
    stationInd = find(statData.stationaritySelection.included_trials.neuronIdx...
        == tempInd);
    ...
    for ssd_i = 1:length(behavior(behaviorIdx,:).stopSignalBeh.inh_SSD)
        ...
        if length(trl_canceled) < trlLim | length(trl_nostop) < trlLim |...
            ...
            sdf_seen_ssdx(ssd_i,:) = nan(1,length(-1000:2000));
            sdf_unseen_ssdx(ssd_i,:) = nan(1,length(-1000:2000));
            ...
            sdf_seen_ssdx(ssd_i,:) = nanmean(z_sdf(trl_seen,:),1);      
            sdf_unseen_ssdx(ssd_i,:) = nanmean(z_sdf(trl_unseen,:),1);      
        end
    end
    ...
    seenInds = vertcat(behavior(behaviorIdx,:).ttseen.NC.S{:});
    unseenInds = vertcat(behavior(behaviorIdx,:).ttseen.NC.U{:});
    sdf_seenAll(neuron_i,:) = nanmean(z_sdf(seenInds,:));
    sdf_unseenAll(neuron_i,:) = nanmean(z_sdf(unseenInds,:));
    NSinds = vertcat(behavior(behaviorIdx,:).ttx.nostop.all.all);
    cancInds = vertcat(behavior(behaviorIdx,:).ttx.canceled.all.all);
    sdf_canceledAll(neuron_i,:) = nanmean(z_sdf(cancInds,:));
    sdf_nostopAll(neuron_i,:) = nanmean(z_sdf(NSinds,:));
    ...
    save(['E:\conflict\results\' region 'seenregression_workspace.mat'])
...
mcc_analysis_table.glm_trialPos = encoding_flag_mcc(:,1) &encoding_beta_mcc(:,1)>0 &...
    encoding_flag_mcc(:,4) &encoding_beta_mcc(:,4)>0;
...
trial_type_neurons = [];
trial_type_neurons = find(mcc_analysis_table.glm_seen == 1);
...
neuron_index.trial_type.facAscend = find(mcc_analysis_table.glm_ssd_cancPos...
    & mcc_analysis_table.glm_trialPos);
neuron_index.trial_type.facDescend = find(mcc_analysis_table.glm_ssd_cancNeg...
    & mcc_analysis_table.glm_trialPos);
neuron_index.trial_type.supAscend = find(mcc_analysis_table.glm_ssd_cancPos...
    & mcc_analysis_table.glm_trialNeg);
neuron_index.trial_type.supDescend = find(mcc_analysis_table.glm_ssd_cancNeg...
    & mcc_analysis_table.glm_trialNeg);
