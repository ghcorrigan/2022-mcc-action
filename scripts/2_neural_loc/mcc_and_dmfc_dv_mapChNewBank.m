...
for session_i = 1:n_sessions
    ...
    if contains(neuralFilename,'ACC')
    ...
    for neuron_i = 1:n_neurons
        ...
        map_info(neuron_i,:) = table(session,behaviour,monkey,unit,site,depth,area,ap,ml,mua,noise,spk_width,probeName,probeNum,probeUnit);
    end
    ...
    area_map_info = [area_map_info; map_info];
end

% Clear up unknown areas and remove noise clusters
area_map_info(strcmp(area_map_info.area,'?'),:) = [];
area_map_info.area(strcmp(area_map_info.area,'Ctx')) = {'DMFC'};
area_map_info(area_map_info.noise == 1,:) = [];
...
fprintf('Nneurons [all], Monkey Da: %i | Nneurons [all], Monkey Jo: %i    \n',...
    sum(contains(area_map_info.monkey,'dar')), sum(contains(area_map_info.monkey,'jou')))
fprintf('Nneurons [DMFC], Monkey Da: %i | Nneurons [dMCC], Monkey Jo: %i    \n',...
    sum(contains(area_map_info.monkey,'dar') & contains(area_map_info.area,'DMFC')),...
    sum(contains(area_map_info.monkey,'jou') & contains(area_map_info.area,'DMFC')))
fprintf('Nneurons [dMCC], Monkey Da: %i | Nneurons [dMCC], Monkey Jo: %i    \n',...
    sum(contains(area_map_info.monkey,'dar') & contains(area_map_info.area,'dMCC')),...
    sum(contains(area_map_info.monkey,'jou') & contains(area_map_info.area,'dMCC')))
fprintf('Nneurons [vMCC], Monkey Da: %i | Nneurons [vMCC], Monkey Jo: %i    \n',...
    sum(contains(area_map_info.monkey,'dar') & contains(area_map_info.area,'vMCC')),...
    sum(contains(area_map_info.monkey,'jou') & contains(area_map_info.area,'vMCC')))
fprintf('Nneurons [ACS], Monkey Da: %i | Nneurons [ACS], Monkey Jo: %i    \n',...
    sum(contains(area_map_info.monkey,'dar') & contains(area_map_info.area,'ACS')),...
    sum(contains(area_map_info.monkey,'jou') & contains(area_map_info.area,'ACS')))
