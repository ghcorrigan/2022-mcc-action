function [glm_output, encoding_flag, encoding_beta] = stopping_glm_func_fr(neuron_i, mcc_map_info, dataFiles_beh, dirs)
...
for timepoint_i = 1:n_times
    ...
    reg_tbl_trialtype.firing_rate = event_frs(reg_tbl.trial_type == 1 |...
        reg_tbl.trial_type == 2,timepoint_i);
...
for timepoint_i = 1:n_times
    ...
    % Input the timepoint specific firing times
    reg_tbl_stopping.firing_rate = event_frs(reg_tbl.trial_type == 2,timepoint_i);
...
for timepoint_i = 1:n_times
    ...
    % Input the timepoint specific firing times
    reg_tbl_canc.firing_rate = event_frs(reg_tbl.trial_type == 2 |...
        reg_tbl.trial_type == 3,timepoint_i);
...
%if noncancelled isn't significant, we don't care about SSD being significant
glm_output.ssd.sig_times(1,glm_output.nonCanc.sig_times(1,:) ==0)=0;
...
%if noncacelled is higher, we don't care
glm_output.nonCanc.sigTime(1,glm_output.nonCanc.beta_weights(1,:)<0) = 0;
