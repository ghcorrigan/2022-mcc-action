function [glm_output, encoding_flag, encoding_beta] = ...
    stopping_glm_func_fr_aligned_baseline(neuron_i, mcc_map_info, dataFiles_beh, dirs,behData)
...
cancSSDs = reg_tbl.ssd(reg_tbl.trial_type ==2);
reg_tbl.ssd(reg_tbl.trial_type ==2) = cancSSDs(randperm(length(cancSSDs)));
for ssd_i = 1:length(matchedBySSD)
   noStopAvg(ssd_i,:) = mean( reg_tbl_trialtype.win_fr(...
       reg_tbl_trialtype.ssd == ssd_i & reg_tbl_trialtype.trial_type ==1,:));
   reg_tbl.fr_diff(reg_tbl.ssd==ssd_i,:) = reg_tbl.win_fr(reg_tbl.ssd==ssd_i,:) -...
       noStopAvg(ssd_i,:);
