function [glm_output, encoding_flag, encoding_beta,inh_pnc,frs,sig_bins,pvals] = ...
    stopping_glm_func_fr_alignedHoNo(neuron_i, mcc_map_info, behData)
rng(1988)
...
reg_tbl.ssd = behData.trialEventTimes{1}.ssdInd;
...
spk_data_in = load(fullfile('E:\conflict\data\NoHo\Spikes\',...
    [neuralFilename '_Spikes_' neuronLabel '.mat']));
window_shift = 10;
fr_window = 100;
event_alignment = 'ssrt';
baseline_win = [-600:-500];
[event_frs,~] = calc_FRs_and_zscore(spk_data_in.Spikes,baseline_win,event_alignment,...
    [-50, 650],fr_window,window_shift);

 n_times = 31; % Get the number of windows in the time averaged data
...
[event_frs_matched, sdfs] = calc_FRs_matched(spk_data_in.Spikes,baseline_win,event_alignment,...
...
for timepoint_i = 1:n_times
   ...
   if any(reg_tbl_trialtype.firing_rate ~= reg_tbl_trialtype.firing_rate(1))
    ...
    glm_output.trial_type.beta_weights(2,timepoint_i) = u_t_mdl.Coefficients.Estimate(3); % trial_number
       ...
       glm_output.trial_type.sig_times(1,timepoint_i) = 0; % trial type
    glm_output.trial_type.pvals(1,timepoint_i) = 1; % trial type
    glm_output.trial_type.beta_weights(1,timepoint_i) = 0; % trial type

    % - Trial number
    glm_output.trial_type.sig_times(2,timepoint_i) = 0; % trial_number
    glm_output.trial_type.beta_weights(2,timepoint_i) = 0; % trial_number
   end
...
for ssd_i = 1:length(matchedBySSD)
   noStopAvg(ssd_i,:) = mean( reg_tbl_trialtype.win_fr(...
       reg_tbl_trialtype.ssd == ssd_i & reg_tbl_trialtype.trial_type ==1,:),1);
...
%     glm_output.ssd.beta_weights(3,timepoint_i) = u_t_mdl.Coefficients.tStat(5); % % ssd x value
    ...
    % - Trial number
    glm_output.ssd.sig_times(2,timepoint_i) = u_t_mdl.Coefficients.pValue(2) < .01; % trial_number
    glm_output.ssd.beta_weights(2,timepoint_i) = u_t_mdl.Coefficients.Estimate(2); % trial_number
...
% figure
% hold on
% for ssd = unique(reg_tbl_stopping.ssdVal)'
%    plot(mean(sdfs(reg_tbl_stopping.trial_number(reg_tbl_stopping.ssdVal==ssd),:),1)) 
% 
% end
% 
% figure
% hold on
% for ssd = unique(reg_tbl_stopping.ssd)'
%    plot(mean(sdfs(reg_tbl_seen.trial_number(reg_tbl_seen.ssd==ssd),:),1)) 
% 
% end
% 
% figure
% hold on
% plot(nanmean(sdfs(reg_tbl_trialtype.trial_number(reg_tbl_trialtype.trial_type==1),:),1))
% plot(mean(sdfs(reg_tbl_trialtype.trial_number(reg_tbl_trialtype.trial_type==2),:),1))
% plot(mean(sdfs(reg_tbl_seen.trial_number(reg_tbl_seen.seen==1),:),1))
% 
% for unit = 
% figure
% hold on
