...
cancSSDs = reg_tbl_trialtype.ssd(reg_tbl_trialtype.is_canc ==1);
reg_tbl_trialtype.ssd(reg_tbl_trialtype.in_canc ==1) = cancSSDs(randperm(length(cancSSDs)));
for ssd_i = 1:length(matchedBySSD)
...
reg_tbl_canc.is_canc = reg_tbl_canc.is_canc(randperm(height(reg_tbl_canc)));
...
reg_tbl_seen.seen = reg_tbl_seen.seen(randperm(height(reg_tbl_seen)));
