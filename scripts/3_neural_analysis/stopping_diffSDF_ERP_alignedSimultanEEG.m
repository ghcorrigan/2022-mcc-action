function [glm_output] = ...
    stopping_diffSDF_ERP_alignedSimultanEEG(sess, ephysLog, dajo_datamap_curated, behData,area)
rng(1988)
neuralFilename = dajo_datamap_curated.lfpFile{sess};
sefNeuralFilename = strrep(neuralFilename,'ACC','DMFC')
sefEEGfilename = strrep(sefNeuralFilename,'lfp','eeg')
EEGfilename = strrep(neuralFilename,'lfp','eeg')
...
mccLFP = lfp;
load(['E:\conflict\data\' sefNeuralFilename])
rightInds = getDir(behData.ttx);
eegFile = 1;

try load(['E:\conflict\data\' EEGfilename])
catch
    try
    load(['E:\conflict\data\' sefEEGfilename])
    catch
        eegFile = 0
...
frs.ssd = {};
frs.pnc = {};
if eegFile==0 || height(reg_tbl(goodtrls,:))==0 %|| sum(reg_tbl.trial_type(goodtrls)==2)<30
...
%% error vs nostop
        ...
        CCephysTableInd = find(contains(ephysLog.Session, neuralFilename(1:end-8)));
        ephysTableInd = find(contains(ephysLog.Session, sefNeuralFilename(1:end-8)));
    ...
    ctxChan =  ephysLog.ctxPran(ephysTableInd);
      ...
      bankChannel = str2num(ephysLog.accPranBank{CCephysTableInd});
     
          lfpdatadcc = cleanLFP_CCFilt(mccLFP,bankChannel,'dACC');
          lfpdatavcc = cleanLFP_CCFilt(mccLFP,bankChannel,'vACC');
      
      if isempty(lfpdata.data)
          return
      end
      if eegFile==1
           eegdata =cleanEEG(eeg,1);
      end


goErrorTrls = reg_tbl.trial_type==1 | (reg_tbl.trial_type==3 & reg_tbl.seen==1);
goErrTrlInds = find(goErrorTrls);
...
allSacst = behData.trialEventTimes{1}.saccade(goErrorTrls);
            ...
            [allERPs,sefBadInds] = cleanERPshuffle(LFPs);

            dccLFPs = arrayfun(@(x) lfpdatadcc.data(:,x-1000:x+2000) ...
                - mean(lfpdatadcc.data(:,x-150:x-50),2),allSacsInds,'uni',0);
            [allERPsd,dBadInds] = cleanERPshuffle(dccLFPs);

            vccLFPs = arrayfun(@(x) lfpdatavcc.data(:,x-1000:x+2000) ...
                - mean(lfpdatavcc.data(:,x-150:x-50),2),allSacsInds,'uni',0);
            [allERPsv,vBadInds] = cleanERPshuffle(vccLFPs);
            if eegFile==1
            eegERPs = arrayfun(@(x) eegdata.data(:,x-1000:x+1000) ...
                - mean(eegdata.data(:,x-150:x-50),2),allSacsInds,'uni',0);
            [allERPseeg,eegBadInds] = cleanERPshuffle(eegERPs);
            allBadInds = unique([sefBadInds;dBadInds;vBadInds;eegBadInds]);
            else
                allBadInds = unique([sefBadInds;dBadInds;vBadInds]);
            end


reg_tbl.ERPs(goErrorTrls,:) = allERPs;
reg_tbl.ERPsD(goErrorTrls,:) = allERPsd;
reg_tbl.ERPsV(goErrorTrls,:) = allERPsv;
if eegFile==1
reg_tbl.EEG(goErrorTrls,:) = allERPseeg;
end
goodtrls(isnan(reg_tbl.ERPs(:,1)))=false;
goodtrls(goErrTrlInds(allBadInds)) = false;
...
glm_output.SEFchans = lfpdata.chans;

%% dcc

[errorCI, errorDiff, ~,actualDataError] = ...
                calculatePermutationContinuityERP(reg_tbl_error.ERPsD,...
                reg_tbl_error.trial_type==1,reg_tbl_error.trial_type==3,contFiller);
noStopSDFErr = mean(reg_tbl_error.ERPsD(reg_tbl_error.trial_type==1,:));
nonCancSDFErr = mean(reg_tbl_error.ERPsD(reg_tbl_error.trial_type==3,:));
nonCancSDFErr2 = nanmean(reg_tbl.ERPsD(reg_tbl.trial_type==3 & reg_tbl.seen==1,:));
noStopSDFErr2 = nanmean(reg_tbl.ERPsD(reg_tbl.trial_type==1,:));
newdiff = nonCancSDFErr2-noStopSDFErr2;


nonCancSDFErrLong = nanmean(reg_tbl.ERPsD(reg_tbl.trial_type==3 & reg_tbl.longseen==1,:));
longDiff = nonCancSDFErrLong - noStopSDFErr;

% 
glm_output.trial_errorGoSDFDCC.CIs = errorCI;
glm_output.trial_errorGoSDFDCC.diff = errorDiff;
glm_output.trial_errorGoSDFDCC.allDiff = newdiff;
glm_output.trial_errorGoSDFDCC.table = actualDataError;
glm_output.trial_errorGoSDFDCC.nostopSDF = noStopSDFErr;
glm_output.trial_errorGoSDFDCC.noCancSDF = nonCancSDFErr;
glm_output.trial_errorGoSDFDCC.noCancSDF2 = nonCancSDFErr2;
glm_output.trial_errorGoSDFDCC.nostopSDF2 = noStopSDFErr2;
glm_output.trial_errorGoSDFDCC.nonCancSDFErrLong = nonCancSDFErrLong;
glm_output.trial_errorGoSDFDCC.longDiff = longDiff;
glm_output.dCCchans = lfpdatadcc.chans;

%% vcc

[errorCI, errorDiff, ~,actualDataError] = ...
                calculatePermutationContinuityERP(reg_tbl_error.ERPsV,...
                reg_tbl_error.trial_type==1,reg_tbl_error.trial_type==3,contFiller);
noStopSDFErr = mean(reg_tbl_error.ERPsV(reg_tbl_error.trial_type==1,:));
nonCancSDFErr = mean(reg_tbl_error.ERPsV(reg_tbl_error.trial_type==3,:));
nonCancSDFErr2 = nanmean(reg_tbl.ERPsV(reg_tbl.trial_type==3 & reg_tbl.seen==1,:));
noStopSDFErr2 = nanmean(reg_tbl.ERPsV(reg_tbl.trial_type==1,:));
newdiff = nonCancSDFErr2-noStopSDFErr2;


nonCancSDFErrLong = nanmean(reg_tbl.ERPsV(reg_tbl.trial_type==3 & reg_tbl.longseen==1,:));
longDiff = nonCancSDFErrLong - noStopSDFErr;

% 
glm_output.trial_errorGoSDFVCC.CIs = errorCI;
glm_output.trial_errorGoSDFVCC.diff = errorDiff;
glm_output.trial_errorGoSDFVCC.allDiff = newdiff;
glm_output.trial_errorGoSDFVCC.table = actualDataError;
glm_output.trial_errorGoSDFVCC.nostopSDF = noStopSDFErr;
glm_output.trial_errorGoSDFVCC.noCancSDF = nonCancSDFErr;
glm_output.trial_errorGoSDFVCC.noCancSDF2 = nonCancSDFErr2;
glm_output.trial_errorGoSDFVCC.nostopSDF2 = noStopSDFErr2;
glm_output.trial_errorGoSDFVCC.nonCancSDFErrLong = nonCancSDFErrLong;
glm_output.trial_errorGoSDFVCC.longDiff = longDiff;
glm_output.vCCchans = lfpdatavcc.chans;

DMFCrms = structfun(@(x) rms(x),lfp.data);
CCrms = structfun(@(x) rms(x),mccLFP.data);
glm_output.dmfcRMS = DMFCrms;
glm_output.ccRMS = CCrms;

%% eeg
if eegFile==1
[errorCI, errorDiff, ~,actualDataError] = ...
                calculatePermutationContinuityERP(reg_tbl_error.EEG ,...
                reg_tbl_error.trial_type==1,reg_tbl_error.trial_type==3,contFiller);
noStopSDFErr = mean(reg_tbl_error.EEG(reg_tbl_error.trial_type==1,:));
nonCancSDFErr = mean(reg_tbl_error.EEG(reg_tbl_error.trial_type==3,:));
nonCancSDFErr2 = nanmean(reg_tbl.EEG(reg_tbl.trial_type==3 & reg_tbl.seen==1,:));
noStopSDFErr2 = nanmean(reg_tbl.EEG(reg_tbl.trial_type==1,:));
newdiff = nonCancSDFErr2-noStopSDFErr2;


nonCancSDFErrLong = nanmean(reg_tbl.EEG(reg_tbl.trial_type==3 & reg_tbl.longseen==1,:));
longDiff = nonCancSDFErrLong - noStopSDFErr;

% 
glm_output.trial_errorGoSDFEEG.CIs = errorCI;
glm_output.trial_errorGoSDFEEG.diff = errorDiff;
glm_output.trial_errorGoSDFEEG.allDiff = newdiff;
glm_output.trial_errorGoSDFEEG.table = actualDataError;
glm_output.trial_errorGoSDFEEG.nostopSDF = noStopSDFErr;
glm_output.trial_errorGoSDFEEG.noCancSDF = nonCancSDFErr;
glm_output.trial_errorGoSDFEEG.noCancSDF2 = nonCancSDFErr2;
glm_output.trial_errorGoSDFEEG.nostopSDF2 = noStopSDFErr2;
glm_output.trial_errorGoSDFEEG.nonCancSDFErrLong = nonCancSDFErrLong;
glm_output.trial_errorGoSDFEEG.longDiff = longDiff;
glm_output.EEGchans = lfpdatadcc.chans;
compareEEG               
end
end
% 
